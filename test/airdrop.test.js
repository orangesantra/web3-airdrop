...
describe("AirDrop", function () {
  ...
  before(async function () {
    ...
    // if (this.shuffle.indexOf(r) === -1) {: It checks if the generated random number r 
    // is not already in the shuffle array. The indexOf function is used to check if r exists 
    // in the array. If r is not found (indexOf returns -1), it means it's a unique number, and it proceeds.

    // Get all signers
    addrs = await ethers.getSigners();
    // Deploy eth swap
    const EthSwapFactory = await ethers.getContractFactory('EthSwap', addrs[0]); // addrs[0] is way od signifing deployer of contract.
    this.ethSwap = await EthSwapFactory.deploy();
    const receipt = await this.ethSwap.deployTransaction.wait()
    contractBlocknumber = receipt.blockNumber // -------------------------------blockNumber is 1

    // Instantiate token
    let tokenAddress = await this.ethSwap.token(); // Token.sol is deployed along with EthSwap.sol via its constructer function.
    this.token = (
    ...
    await Promise.all(this.shuffle.map(async (i, indx) => {
      ...
      expect(receipt.blockNumber).to.eq(indx + 2) // -------------------------------blockNumber is 2
    }))
